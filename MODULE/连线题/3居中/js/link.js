    ...
    for(var i =0 ; i < unique.leftCount; i ++){
        ...
        if(((content.length-temp3)*temp[0]+temp3*temp[0]*0.85) >= (lists[i].clientWidth - temp2[0])){
            if(/(Android)/i.test(navigator.userAgent)){
                lists[i].setAttribute("style","padding-top:3.317%;padding-bottom:3.317%"); 
            }
            else if(/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)){
                lists[i].setAttribute("style","padding-top: 2.82%;padding-bottom: 2.82%;");   
            }
            else {
                lists[i].setAttribute("style","padding-top:2.403%;padding-bottom:2.403%"); 
            }
        }   
    }
