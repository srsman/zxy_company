        ...
        if(linkEve.prototype.dialogShow === undefined){
            linkEve.prototype.dialogShow = function(error, str){
                ...
                setTimeout(function(){
                    ...
                    if(/(iPhone|iPad|iPod|iOS|Android)/i.test(navigator.userAgent)){
                        document.getElementById("submitBtn").setAttribute("style","margin-top:-8.6%;");
                    }
                    else{
                        document.getElementById("submitBtn").setAttribute("style","margin-top:-8.6%;");
                    }
...
//    }
    ...
    for(var i =0 ; i < unique.leftCount; i ++){
        ...
        var temp2 = paddleft.split("p")[0];
        var num = numTemp(lists[i].innerHTML);
        var count = Math.floor(((lists[i].innerHTML.length - num)*temp+ (num*temp*0.8))/Math.floor(lists[i].offsetWidth-2*temp2));
        var lineH = lists[i].offsetHeight;
        lists[i].setAttribute("style","line-height:"+lineH/(count+1)+"px;");
    }
