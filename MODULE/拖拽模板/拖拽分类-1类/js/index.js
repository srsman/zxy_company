...
//    正确答案填写的地方，请示用和一下字符串相同的格式
    ...
    var drag = function(){
        ...
        drag.prototype.showAnswer = function(str){
            ...
            for(var i =0 ; i < num; i++){
                var count = document.getElementById("typeBox"+(i+1)).getElementsByTagName("div");
                var len = count.length;
                for(var j =0 ; j < len; j++){
                    drag.prototype.recovery(count[0]);
                    document.getElementById("typeBox"+(i+1)).removeChild(count[0]);   
                }
            }
            for(var i = 0; i < temp.length ; i++){
                ...
                if(temp2[1]){
                    ...
                    var tempLists = document.getElementById("innerType"+temp[0]).getElementsByTagName("div");
                    var heightTemp = 0;
                    for(var i =0 ; i < tempLists.length; i++){
                        var styless = window.getComputedStyle(tempLists[i],null);
                        var margin = styless.marginBottom.split("p")[0];
                    ...
                    document.getElementById("innerType"+temp[0]).setAttribute("style","height:"+heightTemp+"px");
                }
            }
        }
