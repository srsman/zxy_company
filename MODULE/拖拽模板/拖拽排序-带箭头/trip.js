var loading = function(){
    ...
    var trip = function(){
        ...
        trip.prototype.freshList = function(){
            ...
            var ele = document.getElementById("container").getElementsByClassName("content");
            for(var i = 0 ; i < ele.length ; i++){
                if(ele[i].getAttribute("id")){
        ...
        trip.prototype.getMobilePosition = function(event){
            ...
            var containerX = document.getElementById("container").offsetLeft+document.getElementById("showout").offsetLeft;
            var containerY = document.getElementById("container").offsetTop+document.getElementById("showout").offsetTop;
...
//        鼠标开始或是触屏开始事件
        trip.prototype.mouseDownEvent = function(ele,event){
            ...
            if(/(iPhone|iPad|iPod|iOS|Android)/i.test(navigator.userAgent)){
                ...
                newEle.setAttribute("style","position:absolute; top:"+(po.y-20)+"px;z-index:100;");
            }
            else{
...
//        鼠标移动或是触屏移动事件
        trip.prototype.mouseMove = function(ele,events){
            ...
            if(/(iPhone|iPad|iPod|iOS|Android)/i.test(navigator.userAgent)){
                ele.setAttribute("style","position:absolute; top:"+(po.y-20)+"px;z-index:100;");
            }
            else {
                ele.setAttribute("style","position:absolute; top:"+(po.y-20)+"px;z-index:100;");
            }
...
//        鼠标点击结束或是触屏结束事件
        ...
        trip.prototype.eleOperation = function(ele){
            ...
            newEle.setAttribute("class", "content");
            newEle.setAttribute("id","trip_"+(ele.getAttribute("id")));
            newEle.innerHTML = ele.innerHTML;
            document.getElementById("showout").appendChild(newEle);
            return newEle;
        }
        
        trip.prototype.upOperation = function(ele){
            document.getElementById("showout").removeChild(ele);
        }
...
//        当前位置核对和修改函数
        trip.prototype.checkPostion =function(ele, orderLists){
            ...
            for(var i = 0 ; i < orderlis.length; i++){
                ...
                else if((option.offsetTop + option.offsetHeight/3) >= positionY){
                    if(check == 0){
                        var poOption = document.getElementById("image"+(i+1));
                        var nextOption = document.getElementById("image"+(i+2));
                        document.getElementById("showout").insertBefore(moveEle, poOption);
                        document.getElementById("showout").insertBefore(option, nextOption);
                        number--;
                    }
                }
                else if((option.offsetTop - option.offsetHeight/3) <= positionY){
                    if(check == 1){
                        var preOption = document.getElementById("image"+i);
                        var poOption = document.getElementById("image"+(i+1));
                        document.getElementById("showout").insertBefore(moveEle, poOption);
                        document.getElementById("showout").insertBefore(option, preOption);
                        number++;
                    }
                }
            }
        ...
        trip.prototype.showRights = function(ansStr, styleStr){
            ...
            for(var i = 0 ; i < answerList.length ; i++){
                var move = document.getElementById(""+answerList[i]);
                var poImage = document.getElementById("image"+(i+1));
                document.getElementById("showout").insertBefore(move, poImage);
                if(styleStr){
    ...
    var lists = document.getElementById("container").getElementsByClassName("content");
